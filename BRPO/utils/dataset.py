    ...
    def load_poses(self, path):
        ...
        for i in range(self.n_img):
            ...
            frame = {
                ...
                "mono_depth_path": self.mono_depth_paths[i] if i < len(self.mono_depth_paths) else None,
                "transform_matrix": pose.tolist(),      
            }
            self.frames.append(frame)
...
class MonocularDataset(BaseDataset):
    ...
    def __getitem__(self, idx):  
        ...
        if self.has_depth:
            ...
            mono_depth_path = self.mono_depth_paths[idx] if idx < len(self.mono_depth_paths) else None
            mono_depth = self.load_image(mono_depth_path) / (self.depth_scale*5) if mono_depth_path is not None else None
